...
from models.predFormer_Modules import Attention, PreNorm
...
class PredFormerLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # ts-t branch
        x_ts = rearrange(x_ts, 'b t n d -> (b n) t d',b=b)
...
class PredFormer_Model(nn.Module):
    def __init__(self, model_config, **kwargs):
        ...
        self.num_frames_in = model_config['input_length']
        ...
        self.mlp_head = nn.Sequential(
            ...
            nn.Linear(self.dim, self.output_channels * self.patch_size ** 2)
        )
    ...
    def forward(self, x):
        ...
        x = x.view(B, T, self.num_patches_h, self.num_patches_w, self.output_channels, self.patch_size, self.patch_size)
        x = x.permute(0, 1, 4, 2, 5, 3, 6).reshape(B, T, self.output_channels, H, W)
